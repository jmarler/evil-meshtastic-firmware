...
#endif
#if defined(EVIL_NODE) && defined(EVIL_ALLOW_FROM_OVERRIDE)
    // Evil node: allow the client to specify an arbitrary source node ID.
    // If client left p.from == 0, Router::send() will fill in our own node number.
    // This enables source node ID spoofing from the Python API.
#else
