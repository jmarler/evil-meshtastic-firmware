...
#ifdef EVIL_NODE
void menuHandler::evilMenu()
{
    static const char *optionsArray[] = {"Back", "KILL ALL"};
    enum optionsNumbers { Back = 0, KillAll = 1 };
    BannerOverlayOptions bannerOptions;
    bannerOptions.message = "Evil Control";
    bannerOptions.optionsArrayPtr = optionsArray;
    bannerOptions.optionsCount = 2;
    bannerOptions.bannerCallback = [](int selected) -> void {
        if (selected == KillAll) {
            // Reset all evil features to off/safe defaults
            ::EvilNode::state.transform = 0;
            ::EvilNode::state.drop_rate = 0;
            ::EvilNode::state.drop_node = 0;
            ::EvilNode::state.drop_channel = 255;
            ::EvilNode::state.hop_mode = 0;
            ::EvilNode::state.suffix[0] = '\0';
            LOG_INFO("[EVIL] Panic: all features disabled via OLED menu");
            screen->runNow();
        }
    };
    screen->showOverlayBanner(bannerOptions);
